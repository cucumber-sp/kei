...
describe("Checker — Type Edge Cases", () => {
  describe("const assignment errors", () => {
    ...
    test("increment const → error", () => {
      checkError(
        `fn main() -> int { const x = 5; x++; return x; }`,
        "cannot assign to immutable"
      );
    });
  ...
  describe("function return type errors", () => {
    ...
    test("void return in non-void function → error", () => {
      checkError(
        `fn getInt() -> int { return; }`,
        "expects return type 'i32', got void"
      );
    });
  ...
  describe("comparing different types", () => {
    test("int == string → error", () => {
      checkError(
        `fn main() -> int { let x = 1 == "hello"; return 0; }`,
        "requires same types"
      );
    });
    ...
    test("bool < int → error", () => {
      checkError(
        `fn main() -> int { let x = true < 1; return 0; }`,
  ...
  describe("arithmetic on invalid types", () => {
    ...
    test("string - string → error", () => {
      checkError(
        `fn main() -> int { let x = "a" - "b"; return 0; }`,
        "requires numeric operands"
      );
    });
    ...
    test("string * int → error", () => {
      checkError(
        `fn main() -> int { let x = "a" * 2; return 0; }`,
        "requires numeric operands"
      );
    });
    ...
    test("unary minus on bool → error", () => {
      checkError(
        `fn main() -> int { let x = -true; return 0; }`,
    ...
    test("unary minus on string → error", () => {
      checkError(
        `fn main() -> int { let x = -"hello"; return 0; }`,
        "requires numeric operand"
      );
    });
  });
  ...
  describe("calling non-function", () => {
    test("call integer variable → error", () => {
      checkError(
        `fn main() -> int { let x = 42; let y = x(1); return 0; }`,
        "is not callable"
      );
    });
    ...
    test("call bool variable → error", () => {
      checkError(
        `fn main() -> int { let x = true; let y = x(); return 0; }`,
        "is not callable"
      );
    });
  });
  ...
  describe("nested struct and method access", () => {
    ...
    test("access field on non-struct → error", () => {
      checkError(
        `fn main() -> int { let x = 42; let y = x.foo; return 0; }`,
        "has no property"
      );
    });
  });
  ...
  describe("miscellaneous type checks", () => {
    ...
    test("null assignable to non-ptr → error", () => {
      checkError(
        `fn main() -> int { let x: int = null; return 0; }`,
        "type mismatch"
      );
    });
