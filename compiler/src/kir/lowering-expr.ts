 ...
 */
function resolveStructNameForStaticCall(expr: CallExpr): string {
  ...
  if (genericName) {
    ...
    if (objType?.kind === "module") {
      ...
      const structName = recorded
        ...
        : resolveStructNameForStaticCall(expr);
...
export function lowerMemberExpr(ctx: LoweringCtx, expr: MemberExpr): VarId {
  ...
  if (expr.object.kind === "Identifier" && (objectType?.kind === "struct" || isRefStructObj)) {
    ...
    // Nested member access needs an addressable base for field_ptr.
    baseId = lowerExprAsPtr(ctx, expr.object);
  } else {
    baseId = lowerExpr(ctx, expr.object);
  }
...
export function lowerAssignExpr(ctx: LoweringCtx, expr: AssignExpr): VarId {
  ...
  if (expr.target.kind === "Identifier") {
    ...
    if (expr.target.object.kind === "DerefExpr") {
      ...
      baseId =
        expr.target.object.kind === "MemberExpr"
          ? lowerExprAsPtr(ctx, expr.target.object)
